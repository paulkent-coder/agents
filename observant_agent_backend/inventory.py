...
class ItemManager(StateModule):
	...
	def checkUnderfoot(self, observations):
		...
		# If there are multiple items underfoot, we want to use pickup to check them.
		# That's because the pickup menu tells us the letters we need to use to specify what we want.
		# BUT, before we touch the pickup key, we want to be sure there isn't exactly one item underfoot.
		# If you pickup with 0 items underfoot, it doesn't take up a turn.
		# If you pickup with 2+ items underfoot, you get the pickup menu, which can be cancelled to not use a turn.
			# Even if you're blind – I checked.
		# But if you pickup with exactly 1 item underfoot, you take it and spend a turn.
		# This is an observation-type function – it's not supposed to do anything that causes time to elapse.
		# I don't know whether this agent is robust against a turn passing unexpectedly.
		# I wouldn't count on it – the bot probably will get desynced in some way.
		# So, hopefully, we can count on being able to keep track of whether exactly 1 item is underfoot.
		# That job falls on self.itemUnderfoot. If it's empty, we think there isn't exactly 1 item.
		if self.itemUnderfoot != "":
			self.phase += 2
			return -1
		self.phase += 1
		self.state.get("queue").append(False) # "It's heavy. Pick up anyway?" -> No
		return 61 # pickup
	...
	def readUnderfoot(self, observations):
		if readMessage(observations) != "":
			# There is currently something underfoot if and only if the message is empty.
				# If something is there, a pop-up is on screen, which forces the message to be empty.
				# If we accidentally took 1 thing, that gives a message and no pop-up.
				# If there was nothing there at all, that gives one of a few possible messages, and no pop-up.
			# Therefore, if we see a message, there's nothing to be done.
			self.phase += 1
			return -1
		# There are at least 2 items underfoot. Let's make a note of which of them are worth bothering with.
...
#			return 19 # close the menu
		for entry in display:
			...
			if entry[offset:].find("(10 of ") != -1: # FIXME
				...
				# ^ This seems like a fair assumption to me, that there won't be 10 pages of items on 1 square.
				# But you never know. Might as well keep tabs on that assumption.
				exit(1)
		...
		print("\x1b[0;31mFatal error: A pickup pop up wasn't properly terminated...")
		print(offset)
	...
	def nabGoodies(self, observations):
		if self.itemUnderfoot != "":
			if self.isWorthTaking(observations, self.itemUnderfoot):
				...
				self.itemUnderfoot = ""
				self.state.get("queue").append(True) # "It's heavy. Pick up anyway?" -> Yes
				return 61 # pickup
			return -1 # not worth bothering with
		# Let's check itemsToNab...
		while len(self.itemsToNab) > 0 and self.itemsToNab[-1] == keyLookup[">"]:
			# pop all >s off the back of itemsToNab
			# no sense in scrolling past the page with the last item that we want
			self.itemsToNab = self.itemsToNab[:len(self.itemsToNab)-1]
		if len(self.itemsToNab) > 0:
			self.itemsToNab.append(None)
			self.itemsToNab.append(True)
			self.state.get("queue").append(self.itemsToNab) # queue up item selection key presses
			self.itemsToNab = []
