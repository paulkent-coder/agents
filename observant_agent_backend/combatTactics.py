...
def isWorthFighting(state, observations, monster):
    ...
    if glyph == 27 and observations["blstats"][10] < 21:
        ...
        # At 21 HP, a gas spore explosion has a 2.7% chance to kill us,
        # which I'm willing to put up with.
        # If our HP is any lower than that, let's not take the risk.
        return False

    if glyph == 28:
        # Hitting a floating eye is asking for trouble.
        # TODO: Check if we're blind, because if we're blind
        # we're fine to hit it.
        return False

    if (glyph >= 156 and glyph <= 159) or glyph == 55 or glyph == 56:
        # The molds are sessile and have painful passive counterattacks,
        # so don't antagonize them
        return False
