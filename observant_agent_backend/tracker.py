	...
	def readScreen(self, observations):
		...
		for x in range(len(observations["tty_chars"])):
			...
			if observations["glyphs"][row][col] == 271:
				# Aargh. It turns out aligned priests can't be named, but they also aren't unique.
				# I don't have a good solution for this, but we have to do something,
				# or else the agent will throw every name it has at the priest and then panic.
				# So, we'll make the agent falsely believe there's only one aligned priest.
				# This comes with a couple downsides, but it'll suffice for now,
				# especially since it's unlikely the agent will see more than one at a time.
					# Downside 1: If the agent statuses 1 aligned priest, it'll think future aligned priests are statused.
					# Downside 2: When the agent sees a new priest, it'll forget where it saw the previous priest.
